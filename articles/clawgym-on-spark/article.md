...
:::define[Persona-driven task synthesis]
...
:::math[LoRA's 0.53% trainable rate]
40.4 M trainable / 7.66 B base = 0.53 %. At rank `r` for an attention matrix of shape `d × d`, LoRA replaces a `d²` weight update with `2·d·r`. For `d = 4096, r = 16`: `2 · 4096 · 16 = 131 K` per matrix vs. `16.7 M` full-rank — a 128× compression on the gradient surface. That's why the optimizer state fits, the checkpoint is megabytes, and the train job fits in 114 seconds on Spark.
:::

The trajectories were generated by NIM-served Llama 3.1 8B (FP8-quantized via TensorRT-LLM ModelOpt). That weight format isn't HF-loadable without a re-quant step, and Llama 3.1 8B is an HF-gated model — no `HF_TOKEN` was configured. Qwen 2.5 7B Instruct was already cached bf16 (15 GB) inside the Spark's `tllm-build` container from the [test-time-distilling article](/field-notes/test-time-distilling-for-exploration/) and loads cleanly through transformers. So Phase 4 became **a format-transfer experiment by accident**: train Qwen on Llama-flavored trajectories, ask whether the format-level signal (one bash block per turn, observation-shape responses) survives the cross-base jump.
...
:::why[Format transfer is what SFT actually teaches a 7B agent]
Cross-base SFT (Qwen 7B trained on Llama 8B trajectories) shouldn't have worked if the adapter were learning *bash knowledge* — the bases tokenize differently and have different latent priors over commands. It worked because the signal SFT carries is *protocol shape*: one bash block per turn, observation goes here, `TASK_COMPLETE` lives there. Format is general; world-knowledge isn't. Treat SFT corpora as *behavior demonstrations*, not *fact transfusions*, and the cross-base case stops looking weird.
:::
...
:::pitfall[The adapter inherits whatever the trajectories did, including the bad parts]
The 42-record SFT corpus was filtered for high *per-assertion pass rate*, not for *clean-stop*. Llama 8B PASSes in Phase 3 mostly hit `max_turns=12`, so the model fed Qwen a stylistic instruction: *"keep emitting bash, don't stop"*. The adapter learned exactly that, then lost a fast-base PASS by spinning ten extra turns. SFT data is implicit reward shaping — every property of the demonstrations becomes a thing the student copies. Audit your filter against the behaviors you don't want, not just the ones you do.
:::
...
:::deeper
- [LoRA paper (Hu et al., 2021)](https://arxiv.org/abs/2106.09685) — the rank-decomposition trick that makes a 165 MB adapter a real artifact, with the original ablations on rank vs. recovered accuracy.
- [vLLM LoRA serving docs](https://docs.vllm.ai/en/stable/features/lora.html) — `--lora-modules` plumbing for hot-routed adapters in one server, the primitive Phase 5 leans on.
- [Self-Instruct (Wang et al., 2022)](https://arxiv.org/abs/2212.10560) — the persona-driven synth idea's intellectual ancestor; the paper that established LLM-bootstrapped task generation as a recipe.
- [trajectory-eval-is-the-agent-flailing](/field-notes/trajectory-eval-is-the-agent-flailing/) — sibling article on the per-trajectory thrash signal that made the +15 pp delta interpretable rather than mysterious.
:::
...
:::hardware[Parallel sandboxes scale with unified memory, not GPU count]
Spark trains the rank-16 adapter in 114 s wall, eval-rolls one task at 28 s, and holds base + adapter + 8 K context comfortably in 128 GB unified. An H100 80 GB cuts the adapter train to ~30 s and per-task wall to ~9 s — but on a single H100 you'd still serialize sandbox rollouts because the dual base+adapter footprint is tight on 80 GB. An H200 (141 GB) restores the side-by-side load without dropping batch size; on a B200 (192 GB) you can hold base + 4 adapters and route between them per request. The frontier knob isn't FLOPs — it's enough memory to keep base and adapter (and an 8-task rollout pool) all resident, which is exactly the budget Spark is built around.
:::
