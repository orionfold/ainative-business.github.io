...
Runtimes are the execution backends that run agent tasks and power chat conversations. `ainative-business` supports five runtimes across three providers (Anthropic, OpenAI, Ollama). The Runtimes API exposes the runtime catalog, smart routing suggestions, and Ollama model management.
...
`ainative-business` ships with five agent runtimes. Each runtime has different capabilities:
