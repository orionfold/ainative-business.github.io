...
2. `ainative-business` resolves which profile and project context should be applied to the run.
...
Your agent fails halfway through a large run. Because the execution record lives inside `ainative-business`’s operational surfaces, you can review the monitor output, inspect the task, fix the issue, and resume from a known state instead of starting blind.
