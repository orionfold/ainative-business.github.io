...
Operators typically create tasks from the dashboard or from within a project. Once a task is queued, `ainative-business` hands it to the execution layer, which applies the configured agent profile, logs runtime events, and surfaces permission requests when the agent needs approval.
