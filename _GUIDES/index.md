...
> **Sibling docs live elsewhere** (not here): `_FLOWS/the-machine-that-builds-machines.md`
> (the canonical origin-instruction → artifact map + roadmap, anchor of the `_FLOWS/` stream)
> and root `HANDOFF.md` (the living session-transfer doc). Skill-generated reports stay in the root:
...
|---|---|---|
| [`narrative-contract.md`](narrative-contract.md) | 2026-05-23 | The 11-rule publish rubric for every Orionfold artifact across HF cards + site detail pages (positioning-first, bounded drift, no roadmap language, sibling cross-links, Methods wire-back, data-driven visuals, notebooks-after-positioning). Enforced by `hf-publisher` + `scripts/verify_artifact_rendering.mjs`. |
| [`product-articles.md`](product-articles.md) | 2026-05-29 | The `products/<slug>/product.md` launch-article genre — schema (build-metrics block + feature tour) and the rendering it requires. Authored by `product-writer`. |
