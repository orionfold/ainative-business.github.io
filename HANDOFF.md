...
- Immediate Advisor proof next step: the exemplar-format pass (option a) is DONE — **30B-A3B `/no_think` + format exemplar = 7/8, meeting the §14 ≥7/8 pre-SFT bar without training**. Standings: Qwen2.5 5/8 (comparison, pre-exemplar), Nano 9B `/no_think` 1/8, 30B-A3B 7/8. Next-session decision is now about closing the last row and the remaining §14 gates, in order: (1) decide whether the 0082 grounding over-refusal class warrants SFT (§13.D — 30B-A3B is MoE, NeMo SFT path unproven; spec assigns refusal *boundary calibration* to SFT, and further prompt tuning is off the table) or whether the proof proceeds to the §13.F routing probe with the 7/8 receipt as the documented pre-SFT floor; (2) re-run RAG recall through the live Cortex/Arena lane (local baseline `source_recall@5=0.9885`, `1.0` held-out answerable) before any publish claim; (3) fix AD-AE-14 so the Arena run context says Advisor, not Kepler/astrodynamics (an explicit §14 gate); (4) optional cheap probe of `NVIDIA-Nemotron-3-Nano-4B-BF16` (OA-NV-2) with the same GGUF-recipe + exemplar prompt if a trainable dense fallback is wanted before committing SFT effort to the MoE.
