...
Last recorded runtime baseline: Advisor Nano 9B visible preflight, 2026-06-09.

- Cockpit `:7866` is up in browser-use mode via `.claude/skills/arena-lifecycle/scripts/arena_lifecycle.sh restart --browser`; visible CDP Chromium `:9222` was smoked on `/arena/cortex/` after the `/no_think` control bake. Last restart pids: cockpit `957089`, browser `957106`.
...
- Current GPU lane: cached `fk-nim-8000` Docker NIM container on `:8000`, image `nvcr.io/nim/nvidia/nvidia-nemotron-nano-9b-v2-dgx-spark:latest`, serving model `nvidia/nemotron-nano-9b-v2` with `/v1/models` max length 131072. Arena active lane is pinned in LaneTruth as source `registry`, run-context anchored at `2026-06-09T22:46:19Z`, but the global run context still says Kepler/astrodynamics. Qwen2.5 on `:8091` is torn down.
...
- Tear down when done: stop the NIM lane (`docker stop fk-nim-8000`) before starting another model lane, then `.claude/skills/arena-lifecycle/scripts/arena_lifecycle.sh down --browser` if the cockpit/browser should close too.
...
- Immediate Advisor proof next step: the §13.C step-5 reasoning-suppression probe is DONE — Nano 9B `/no_think` scored 1/8 with zero thinking leakage and SFT-shaped remaining failures (exact source-id format, refusal wording, one grounding over-refusal). Receipts: Qwen2.5 5/8, Nano 9B default 0/8, Nano 9B `/no_think` 1/8 — no Advisor preflight pass yet. Decision now: proceed to the §13.D NeMo SFT probe on Nano 9B (its failures are exactly the SFT-owned classes; NeMo-RL trained, NeMo path documented) vs. first probing `NVIDIA-Nemotron-3-Nano-4B-BF16`/`nemotron-3-nano-30b-a3b` for a better raw floor. Note Qwen2.5's 5/8 raw floor exceeds Nano 9B's; spec §8 weights NVIDIA-native fit at 20% — keep the comparison honest. RAG recall has a passing local baseline (`source_recall@5=0.9885` overall, `1.0` held-out answerable) but should later be repeated through the live Cortex/Arena lane before publish claims.
