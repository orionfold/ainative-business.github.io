...
- RAG/recall gate passing locally: `score_recall.py` → `rag-recall-v0.1.json`; source_recall@5 = 0.9885 all rows, 1.0 held-out answerable. Repeat through the live Cortex/Arena lane before publish claims.
...
- Browser-use evidence staged under `/tmp/orionfold-advisor-dogfood/` (latest: `arena-cortex-nemotron3-30b-nothink-exemplar-7of8.png`; prior lane/default/nothink shots also there). Open dogfood findings are tracked in Open Items (AD-AE-11/12/14, WB-11; AD-AE-13 built).
...
- Cockpit `:7866` is up in browser-use mode; visible CDP Chromium `:9222` was driven for the 30B lane launch, all Cortex preflight runs, and the Advisor build-spine smoke. Last restart pids: cockpit `1144362` (restarted twice on 2026-06-09 to pick up the FK_ARENA_* env anchors + the reward-reader fix; plain `restart` leaves the browser alone), browser `957106`. `.env.local` now carries `FK_ARENA_BUILD_DIR`/`FK_ARENA_CORPUS_DIR`/`FK_ARENA_SFT_DIR`/`FK_ARENA_REWARD_DIR` pointed at the Advisor proof — remove or repoint when the active vertical changes (Kepler's reward/build panes read astro dirs again once unset).
...
- Immediate Advisor proof next step: DECIDED (operator approved 2026-06-09) — **defer SFT, proceed to the §13.F routing probe with the 7/8 receipt as the documented pre-SFT floor**. Rationale: the sole failure (0082 grounding over-refusal) is safe-direction, SFT on an unproven-MoE path for a 1-row class fails the headroom discipline, and governed escalation on a local refusal-of-answerable is exactly the harness's job. SFT re-enters only if a wider slice shows the over-refusal class is systematic. Remaining order: (1) design + run the §13.F route bakeoff (T1 local 30B lane → governed escalation tiers, cost ledger, route accuracy); (2) re-run RAG recall through the live Cortex/Arena lane (local baseline `source_recall@5=0.9885`, `1.0` held-out answerable) before any publish claim; (3) optional cheap probe of `NVIDIA-Nemotron-3-Nano-4B-BF16` (OA-NV-2) with the same GGUF-recipe + exemplar prompt as a trainable dense fallback. AD-AE-14 is FIXED: operator-authored `evidence/orionfold-advisor/build-manifest.json` + `FK_ARENA_BUILD_DIR`/`FK_ARENA_CORPUS_DIR`/`FK_ARENA_SFT_DIR`/`FK_ARENA_REWARD_DIR` anchors in `.env.local` (sourced by `arena_lifecycle.sh`) — run context, build spine, and bench provenance now say Advisor/advisor-bench through the visible cockpit.
- During the run, carry forward the existing `AD-FK-*` fieldkit and `AD-AE-*` Arena findings into the Workbench/Advisor task breakdown rather than treating the superseded dogfood spec as the active plan. Treat terminal-only workarounds as dogfood findings unless they are expected external setup. Current concrete Arena findings: `AD-AE-11` for missing manifest/bench/recall visibility, `AD-AE-12` for missing base-model scout/preflight visibility, `AD-AE-13` built for preflight receipt/lane-readiness/execution, `AD-AE-14` FIXED (Advisor run-context via build-manifest + FK_ARENA_* env anchors; reward-reader env-anchor shipped in fieldkit `[Unreleased]`), **`AD-AE-15` NEW: the build-spine RLVR stage reads the newest `rl_run` row from the shared arena.db with no vertical filter, so Kepler's `rl-000 · 96% held-out` renders as DONE under the Advisor spine — report≠reality; needs result-carried vertical filtering (no schema change) or a per-vertical job scope**, and Workbench `WB-11` because visible LaneTruth can discover/pin NIM lanes but cannot launch them through the guarded UI.
