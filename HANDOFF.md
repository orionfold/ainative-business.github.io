...
- **Open follow-up**: the launch shipped HF dataset `Orionfold/Advisor-bench`, but no `kind: bench` manifest exists for it in the catalog (sibling precedent: `patent-strategist-bench-v0.1.yaml`) — authoring one needs the real shapes/results from the evidence dir, deliberately not invented this session.
