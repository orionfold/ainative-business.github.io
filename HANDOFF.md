...
- §13.C raw-floor receipt history, all run through visible Cortex (receipts overwrite `advisor-preflight-v0.1*.json` in place; priors in git history): Qwen2.5-7B Q5_K_M 4/8 → 5/8 after boundary tightening (comparison baseline); Nano 9B v2 NIM 0/8 default (7/8 thinking leak) → 1/8 `/no_think` (0 leak); **Nemotron-3-Nano-30B-A3B Q8_0 0/8 default (8/8 leak, citations mostly correct underneath) → 5/8 `/no_think` (0 leak, 0 private-state risk) — best NVIDIA-native floor**. Universal remaining failure classes: exact-source-id aliasing (`Source 2`) and bare refusals without refusal wording. No publishable pass yet; do not claim any base selected.
...
- Browser-use evidence staged under `/tmp/orionfold-advisor-dogfood/` (latest: `arena-models-nemotron3-30b-lane.png`, `arena-cortex-nemotron3-30b-default-run.png`, `arena-cortex-nemotron3-30b-nothink-rerun.png`). Open dogfood findings are tracked in Open Items (AD-AE-11/12/14, WB-11; AD-AE-13 built).
