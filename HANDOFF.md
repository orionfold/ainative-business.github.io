...
- **Live** at `ainative.business` (GitHub Pages). `origin/main` tip = `b2bda69`; **local `HEAD` = `6cdefd4`, one commit ahead and not yet pushed** — that commit is the skill-path-drift fix (`compute_stats.py` repoint + `_webui` exclusion, `refresh_readme.py` deleted, LOC KPI simplified, regenerated `src/data/field-notes/project-stats.json`). **Needs a push** (solo-blog direct-to-main; harness denies the first main-push of a session — expect a prompt). **Uncommitted this session (roadmap/spec harvest):** `WORKFLOWS.md` (§3/§7 grounded against measured evidence) + new `_SPECS/roadmap-reconciliation.md` — doc-only, awaiting user review/commit. `WORKFLOWS.md` + `HANDOFF.md` are both **tracked** (WORKFLOWS un-ignored in `adb1c04`). Two pre-existing untracked items remain (`.claude/scheduled_tasks.lock`, `src/data/arena-mirror/` — leave alone).
...
- **Method note (matters for next time):** the Second Brain MCP path was **unusable** — its index held **12/63 articles and none of the roadmap-relevant ones** (stale, lagging commits). Reading fell back to disk via 3 read-only Explore agents (Workflow was *not* opted into). **➜ Action: re-index the Second Brain corpus** before the next harvest that wants to dogfood it.
