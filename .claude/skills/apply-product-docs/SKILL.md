...
- **SEO contract — frontmatter `title:` stays short, `description:` is 70–160 chars** (Google snippet window, verified by `.claude/skills/seo-monitor/scripts/audit_site.mjs`). The site-wide layout `src/layouts/DocsLayout.astro` adds the suffix ` — ainative Documentation`, which puts every rendered docs title in the 25–65 char window. When scaffolding a new docs MDX page (Step 7's "create new page" branch), set `title:` to a short page name only (e.g., `"Apps"`, not `"Apps — Documentation"`) — the suffix is layout-added. Added 2026-05-16 after seven `/docs/*` pages flagged too-short rendered titles (e.g., `"Apps — ainative Docs"` at 20 chars).
