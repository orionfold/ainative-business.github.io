...
`PRODUCT-ARTICLES.md` at the repo root is the **contract to the destination
...
4. **Destination contract:** `PRODUCT-ARTICLES.md` at the repo root (schema + rendering ownership).
...
2. Refresh project stats if the repo's stats pipeline counts products
   ...
   the stats refresh — coordinate via `PRODUCT-ARTICLES.md`, since the
...
- **No `products/` tree yet?** `new_product_article.sh` creates the folder; the
  destination renders it once `PRODUCT-ARTICLES.md`'s schema is wired on the Mac
