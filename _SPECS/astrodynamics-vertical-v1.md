...
| **AV-12** | **RL-headroom gate — BETWEEN SFT-init (C2) and the RLVR run (C5)** *(added 2026-06-05, lesson from the C5 null)* | After C2(b), score the SFT-init on the **exact held-out RL will select against**. **GO only if that score sits in the ~30–70% "Goldilocks band."** ≥~85% → **no headroom, skip the 5.9 h run** (ship SFT or rebuild the selection held-out per AV-R6). ≤~15% → **no reward density**, GRPO collapses to zero-advantage (rebuild easier or improve SFT first). | The C5 null: the 24-row in-loop held-out was **0.958 at step 0** and the 44-row external set **86.36%** — both saturated → RL had nothing to climb (5 degenerate zero-advantage steps; `selected_step=0`). The predictor was in hand at C2(b) and was treated as a footnote. RLVR **amplifies existing competence**; it needs the init *partially* right — not ~100% (no headroom) and not ~0% (no signal). Cheap (inference-only, ~one preflight). |
...
| **AV-R6** | **Selection-held-out saturation** *(realized in C5 — the null)* — the held-out RL selects on is already aced by the SFT init → no headroom → RL shows no lift even though the loop ran perfectly | **realized** (med for any vertical) | a clean 5.9 h null; a flat lineage delta chart | **AV-12 RL-headroom gate** (score SFT-init on the selection set first; require the ~30–70% band). Design the selection held-out **error-mined** — concentrate it on the init model's measured weak spots (here: `hohmann_transfer` 3/6 misses, `altitude_from_period` 2/6), *not* merely "harder" (the external curveballs were harder/off-template and still scored 86%). Keep a **separate** generalization held-out for the publish claim (its requirement is the opposite — honest + representative, not adversarial). | if even an error-mined set saturates, the SFT model is the deliverable — ship it (AV-11) and skip RL; record the null honestly |
...
| Date | Change | Author |
|---|---|---|
| 2026-06-05 | **AV-12 gate RAN (C6 STEP-1.5) — result BIMODAL, ship-vs-redesign fork open.** Built the error-mined Tier-1 transfer set (`scripts/astro_bench/{transfer,gen_transfer,headroom_gate}.py`; 48 candidates, disjoint from pool+heldout+SFT-corpus, 47 tests green) and scored the frozen SFT (`merged-hf-bf16`) on it (NeMo container, max_new_tokens=4096): **agg reward 20.83% (10/48) · boxed 100% · trunc 0%** (`evidence/astrodynamics/av12-headroom-transfer.json`). The error-mine dropped the saturated 86% init to ~21% (clean reasoning misses, no AV-R1). **Per-family BIMODAL:** fully-generalizes some (`xfer_altitude_to_speed` 100%), **0%** on the target weak spots (new-body hohmann/circular, un-named `altitude_from_period`); only **4 families / 15 rows** in the productive (0,1) band (`astro-rl-selection-heldout.jsonl`). Refinement of AV-R6: a **0% family is the mirror of saturation** (zero group-advantage) → an SFT-coverage gap, NOT RL headroom; the selection held-out must be families with pass-rate strictly in (0,1). Caveat: preflight is single-sample → under-estimates GRPO headroom. Fork (STEP-2, operator's call): (1) ship SFT + null, (2) expand SFT corpus for the 0% families, (3) re-RL on the 15-row partial band. Code+evidence (not docs-only). | Manav (with Claude) |
| 2026-06-05 | **+AV-12 (RL-headroom gate) + AV-R6 (selection-held-out saturation), from the C5 null.** C5 ran a clean end-to-end RLVR loop but showed **no lift** — the selection held-out (in-loop 24-row 0.958@step-0; external 44-row 86.36% at C6) was already saturated by the strong 86% SFT init, so GRPO had no headroom (5 degenerate zero-advantage steps, `selected_step=0`). Lesson: the predictor was computable at C2(b) and under-weighted. AV-12 makes it a **GO/NO-GO gate between C2 and C5** — score the SFT-init on the exact selection set, proceed only in the ~30–70% Goldilocks band (RLVR amplifies existing competence; needs partial-right, not ~100% [no headroom] or ~0% [no signal]). AV-R6 adds the **error-mined selection held-out** design (concentrate on the init's measured weak spots — `hohmann_transfer`, `altitude_from_period` — not merely "harder"), kept **separate** from the generalization held-out. Generalized into `rlvr-loop-v1.md` (RV-11) + memory `[[feedback_rlvr_headroom_gate]]`. Docs only. | Manav (with Claude) |
