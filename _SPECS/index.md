<!-- _SPECS index — project specs, plans, and historical design docs. Last updated: 2026-06-04 -->
...
| [`arena-enhancements-v1.md`](arena-enhancements-v1.md) | **DRAFT** (decisions PROPOSED 2026-06-04, unbuilt — confirm before build) | The **dogfood payoff** — formalizes the gitignored ledger `_IDEAS/arena-dogfood-feature-extraction.md` (AF-1/2/4/5/6/7/8/11; AF-3/9/10 already built) **plus** a cockpit **information-architecture refresh** raised after the first live `rl_run`. 14 decisions AE-1…14 in 5 clusters: **A** RL-run observability (AF-11 wire the reward gauge to the live run · degenerate-step visibility · per-step `step_history` · lineage step-index), **B** build-spine (`/arena/build/` · corpus feed · gate cards), **C** provenance (bench card · rl_run lineage threading), **D** wiring (scout→Compare + lock-time behavioral gate · bench preview), **E** information architecture (flow-based nav regroup · data-flow routing audit · Standup/Lab/Reward purpose redefinition). No arena.db churn (`user_version 6`); every fix lands in `result_json`/file-reports/existing-panes/nav. 6-session breakdown §6, **queued in HANDOFF after astro C6**. Release gate ~`fieldkit v0.23.0+`. Awaiting green-light. |
