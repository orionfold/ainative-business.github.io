...
| ID | Decision | How | Why / note |
|---|---|---|---|
| **AE-25** | **SFT canonical progress feed** *(AF-21 / OBS-1 / BUG-1)* — ✅ **BUILT v0.28.0** | `fieldkit.training.run` emits a structured `sft-progress-<run>.json` heartbeat from its existing `on_progress(latest_iter, iter_dirs)` callback + reads `latest_checkpointed_iteration.txt` as ground truth; `/arena/sft/` reads the feed, **invocation-independent**. The operator-armed `sft_run` dispatch follow-on landed as **AE-29** (cut 3). | Fixes the **high-severity** BUG-1 (a completed run rendered `0/0`). Mirrors AE-1 reward-signal + AE-6 corpus heartbeat. |
| **AE-26** | **Inventory truth on the build spine** *(AF-19 / OBS-2)* — ✅ **BUILT cut 3** | Each manifest stage may declare `artifacts: [{path, rows?, files?}]`; `/api/build` verifies them ON DISK at read time (exists · line-count vs claim · dir file-count · bytes · mtime — the AE-8 pattern generalized) → per-stage `inventory` facet + `<BuildSpine>` chips. Binaries stat'd, never read. | Assertion → observation for the no-live-feed stages (P1). |
| **AE-27** | **Corpus-gen request handshake + liveness** *(AF-22 / OBS-3)* — ✅ **BUILT cut 3** | `POST/GET/DELETE /api/corpus-request` — one atomic intent file beside the heartbeats (GS-1 pattern) the CC-session synth skill polls + fulfils (AE-R3 holds); fulfilment = a heartbeat newer than the request, **observed**. Producer liveness = heartbeat-mtime freshness (`live/stale/done/none`, window `FK_ARENA_CORPUS_LIVE_S`) on the corpus stage + the Build-pane handshake block. | Operator can *intend* corpus-gen from Arena; "running but not stamping" is no longer indistinguishable from "no synth." |
| **AE-28** | **Feed self-description / source-health disclosure** *(AF-23; AF-18 broad brief deliberately skipped)* — ✅ **BUILT 2026-06-09** | A compact collapsed `FeedHealth` disclosure on file/report-backed live panes: SFT, Reward, and Corpus. Each names source filename, source kind, last stamp, producer, read path, poll cadence, and health state. The broad operator-brief/checklist pane was reviewed and skipped as low ROI because provenance, runtimes, corpus liveness, and run-context are now surfaced in-place. | Closes the practical "what does this pane read, and is it stale?" gap without adding a duplicate runbook pane. |
| **AE-29** | **Operator-armed `sft_run` dispatch** *(AF-21 dispatch half)* — ✅ **BUILT cut 3** | `JobKind.SFT_RUN` + curated `mcp.run_sft_training(recipe_path, mode)` (`TrainRecipe.from_yaml` → `training.run`, the AE-25 heartbeat feeds the pane free). **Armed twice over:** async-only at `POST /api/jobs` (the rl_run shape) + a **drain brake** — released back to `queued` (audited) unless the draining process exports `FK_SFT_RUN_ARMED=1`; `claim_next_job(skip_ids)` keeps the held job from starving the queue. Jobs-board arm form + `⏸ awaiting armed drain` cue. | Closes "the build pane frames the stage but you still run it in bash" — with the read-only-build safety choice intact (guarded, explicit, operator-armed; never auto-dispatch). |
| **AE-30** | **Runtime readiness — read-only** *(AF-20 observation half)* — ✅ **BUILT cut 3** | `GET /api/runtimes`: serve lanes via AE-18 discovery, training containers via one short-timeout `docker inspect` (`up/stopped/absent/unknown`; roster env `FK_ARENA_RUNTIME_CONTAINERS`), pgvector/embedder via TCP. Cached ~8 s (AE-R7); docker-less box degrades to `unknown`. `<BuildSpine>` Runtimes roster. | The S1 "single biggest gap," observation half (P1). The guarded **arm/teardown half stays deferred** to the AE-22 launch-runner cut — same AE-R13 risk class. |
...
- **V3 — AE-25** SFT canonical feed (fixes BUG-1) + the optional `sft_run` dispatch.
  *(AE-25 shipped early, in the v0.28.0 bug-fix cut.)*
- **V4 — AE-26 inventory truth + AE-27 corpus handshake + AE-28**.
  *(Cut 3 delivered AE-26 + AE-27 + AE-29 sft_run dispatch + AE-30 runtime-readiness
  read-only. AE-28 later landed as the narrow source-health disclosure on SFT / Reward / Corpus; the broad operator-brief pane was intentionally skipped as redundant. The guarded-launch work — AF-20 arm/teardown + AE-22 launch — shipped as AE-31.)*
...
| Date | Change | Author |
|---|---|---|
| 2026-06-09 | **AE-28 BUILT (narrow value-add slice).** Added a shared collapsed `FeedHealth` disclosure and wired it into SFT, Reward, and Corpus surfaces. It exposes source filename, source kind, last stamp, producer, read path, poll cadence, and health state. Browser-smoked live over CDP on `/arena/build/`, `/arena/sft/`, `/arena/reward/` with no console errors. Broad operator-brief/checklist pane was skipped as redundant with in-place provenance, runtime readiness, and corpus liveness. | Manav (with Codex) |
| 2026-06-06 | **Cut 3 BUILT (Cluster I core — observation + dispatch; every guarded-launch risk deferred).** AE-26 inventory truth (manifest `artifacts` declarations verified on disk at read; Kepler manifest declares the real corpus/bench/publish artifacts — chips read `600/600 ✓ · 120/120 ✓ · 8.7 GB ✓` live); AE-27 corpus handshake (`/api/corpus-request` intent file + heartbeat-freshness liveness; live smoke drove request→synthetic-heartbeat-fulfils→withdraw→honest revert); **AE-29** (new) operator-armed `sft_run` (async-only + `FK_SFT_RUN_ARMED` drain brake + `claim_next_job(skip_ids)` no-starvation; smoke queued via the UI form, verified the held cue, cancelled clean); **AE-30** (new) runtime readiness read-only (`/api/runtimes` — lanes/containers/TCP observed; live roster matched the box exactly: pgvector up · ps-train stopped · nemo-train absent · lanes free). Offline 1445 pass / 19 skip (+26). AF-20's arm/teardown + AE-22's launch half → the dedicated launch-runner cut (AE-R13). | Manav (with Claude) |
