...
| Date | Change | Author |
|---|---|---|
| 2026-06-09 | **Applied the 0082 bench erratum and re-ran every receipt the bench hash touches — the §13.C preflight gate PASSES 8/8 for the first time.** Erratum (operator-approved "recommended" path): `generate.py` partner-path loop special-cases `article_nemo_framework_on_spark` — the held-out row keeps the family's separation intent but asks which Orionfold product owns scoring/comparison/leaderboard and expects `product_orionfold_arena` (whose page states it; BM25 rank 1, dense rank 1). Regenerated to /tmp and adopted ONLY the held-out file (pool byte-identical; task id + split stable; new `bench_sha256_12 = 3220b8e799cd`). NOT adopted: the regenerated manifest drifted for unrelated reasons (2 new public specs + sha refreshes on edited docs) — known corpus-freshness drift, left for a deliberate manifest rebuild. Receipt reruns: BM25 recall 0.9885@5 / held-out 1.0 (0082 hit rank 1); live Cortex recall 0.977@5 / held-out 1.0 (dense rank 1); §13.C `/no_think` preflight through the visible Cortex `run /no_think` control → **8/8, 0 leak, 0 private-state risk — gate PASSED, first publishable-grade generator receipt** (0082 answers exactly: "…Orionfold Arena… scoring concerns should be kept separate from training/export evidence. Citations: [product_orionfold_arena]"); §13.F route bakeoff rerun → **T1-only 27/28 (96.4%), refusals 16/16, 0 escalations, $0 hosted** (the router found nothing detectable to escalate; hosted tiers idle). Sole remaining failure: 0040 wrong-citation (`spec_notebooks_as_artifacts_v1_mac`), the documented undetectable-without-labels class — the planned deterministic cited-id∉retrieved-set harness check would catch exactly this. Screenshot: `/tmp/orionfold-advisor-dogfood/cortex-nothink-post-erratum.png`. | Manav (with Claude) |
| 2026-06-09 | Granularity-pass assumption CHECKED AND REFUTED before building anything: the §13.F frontier-consensus hypothesis was that 0082's failure was excerpt-window drift (the relevant passage exists in the rank-1 source but outside the 900-char window). Deterministic corpus probe says otherwise — `articles/nemo-framework-on-spark/article.md` (4,492 words) contains **zero** occurrences of "Arena", "scoring", "governance", "provenance", or "publish/reject"; the expected answer ("training/export evidence is separate from Arena scoring, governance, provenance, and publish/reject decisions") cannot be grounded in the expected source at ANY excerpt size. 0082 is **bench-expectation drift, corpus-level**, and all three tiers' refusals were *correct* grounded behavior. Consequence: NO excerpt/multi-window change shipped (no measured failure demands it — observe-always/build-rarely; live recall is green, 0040 is a citation class, 0004/0076 recover @10). Recommended fix is a bench erratum on the frozen held-out row — repoint 0082's expected source to one that states the separation (`spec_spark_arena_v1` / `product_arena_control_plane` / `doc_flows_the_machine_that_builds_machines_md` all do), or rewrite the row to what the NeMo article supports, or convert it to an unsupported/refusal row. Frozen-bench edit = operator decision; receipts carry `bench_sha256_12` so any erratum is visible. | Manav (with Claude) |
