...
**Abstraction added.** `fieldkit.reward` (scorer → reward adapter) + `fieldkit.rl` (the GRPO driver — wraps the *hand-rolled REINFORCE-with-KL loop* that actually ran, with Unsloth/NeMo-RL as a fallback lane). **Now written: the standalone `_SPECS/rlvr-loop-v1.md`** (10 locked decisions RV-1…10, decisions-locked + unbuilt; placement confirmed standalone since the engine lives mostly outside `fieldkit.arena`). Two reconciliation wins baked in: the reward's `failure_class` reuses the **already-built `fieldkit.lineage.FailureLabel`** 9-class enum, and the `rl_run`/`requant` job kinds are the **pre-drilled `arena/jobs.py` sockets** this spec promotes to `DISPATCHABLE`. Held-out-every-≤10-steps is a **hard gate** (the 81.8 pp inversion); vLLM is pinned and restart-elimination is the top wall-clock win. Release gate ~`fieldkit v0.20.0`. **This is the last of the four `_FLOWS` §3 spec stubs — all four are now written.**

**Post-roadmap follow-on — RL lane autonomy (the engine becomes self-driving).** With all four phases built + shipped (`fieldkit v0.21.0`, GPU backend vendored), the engine's only remaining gap is that its longest job — an ~8.5 h `rl_run` — is still **human-armed at two chokepoints**: the operator brings up the one vLLM lane the box can hold and arms the overnight cron. The follow-on spec **`_SPECS/rl-lane-autonomy-v1.md`** (DRAFT, decisions proposed 2026-06-03) closes both and makes the unattended run trustworthy: a `fieldkit.arena.lane` arbiter (envelope-gated single-lane bring-up/teardown over the shipped `budget.MemoryEnvelope` + `_rl_gpu_serve.VLLMLane`), **live step reporting** (loop → `jobs.result_json` → a cockpit progress pane with the pool-vs-held-out spark-line), a **telemetry-correlated OOM defense** (a `MemoryWatchdog` over the existing `TelemetryHub` mem sample, aborting *before* the kernel OOM-kills — arena-wide, not RL-only), and an **education layer** that surfaces the published RLVR curriculum (`articles/the-machine-improves-itself` `:::` blocks) so Arena teaches the operator at every step. Overwhelmingly connective tissue — no arena.db churn, no new top-level module; release gate ~`fieldkit v0.22.0`. It promotes the §5 living-model launch once a held-out-winning run draws the real `fieldkit.lineage` delta chart.
...
- **Managed local-knowledge appliance** — **SHIPPED as [Orionfold Cortex](/products/orionfold-cortex/)** (slug `orionfold-cortex`; *the Bet-5 recall layer, productized; rides Phase 1*). The control-plane-driven Second Brain — ingest your own docs + research, index, RAG-eval, query — packaged in the same local cockpit (`fieldkit[arena]`). Dogfooded on this repo's own corpus first, then ship "bring your docs, get a *measured* local RAG you operate, not a SaaS." The **provenance/trust filter + a tracked RAG-eval score** are the differentiators a hosted RAG can't honestly claim.
...
- **The phases are sequenced, not yet built — but the substrate is further along than the original draft claimed.** Phase 0 (Workflow fan-out) needs no new infra. Phase 1 (Arena M8) extends a store that already has the `eval_runs.arq_job_id` socket and dispatches through an MCP harness that **already ships 7 tools** (`build_mcp_server()` is live, not stubbed). Phase 3's reward substrate (all 7 `fieldkit.eval` verifiers) is present. Genuinely absent: the dispatcher glue, the cron layer, hook *expansion* (one `SessionStart` hook today), and the RLVR stack (`fieldkit.reward`/`fieldkit.rl`/`g6_*`). **Phase 1 / Arena M8 is now SHIPPED end-to-end** (`_SPECS/spark-arena-v1.md` §12 spec → BUILT §12.7 → `fieldkit v0.16.0` → launch article `606333d`). **All four roadmap spec stubs are now WRITTEN** (decisions locked 2026-06-02, all unbuilt): `cost-plane-v1` = `spark-arena-v1.md` §13 [Arena M9], `second-brain-pipeline-v1` = §14 [Arena M10], `autonomous-harness-v1` = §15 [Arena M11], and the Phase-3 engine `rlvr-loop-v1` = the standalone `_SPECS/rlvr-loop-v1.md` (10 decisions RV-1…10; new `fieldkit.rl` + `fieldkit.reward`). Write order executed (2026-06-02, post-M8): **`cost-plane-v1` ✅ → `second-brain-pipeline-v1` (Bet 5, M10) ✅ → `autonomous-harness-v1` (Phase 2, M11) ✅ → `rlvr-loop-v1` (Phase 3, standalone) ✅** — the two cross-cutting bets ride the now-shipped pane and are *inputs* to a fully-realized Phase 2 (the M11 budget governor reads the Bet-6 ledger; the freshness monitor drives the Bet-5 re-index), which in turn fed the Phase-2 spec; the engine landed last, into everything above. **The spec-authoring backlog is now empty — the next acts are *builds*, not *writes*** (release gates ~`v0.17.0` M9 / ~`v0.18.0` M10 / ~`v0.19.0` M11 / ~`v0.20.0` RLVR). See `_SPECS/index.md` (Planned queue, all ✅) + `HANDOFF.md`.
