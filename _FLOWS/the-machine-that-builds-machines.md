...
- **The phases are sequenced, not yet built — but the substrate is further along than the original draft claimed.** Phase 0 (Workflow fan-out) needs no new infra. Phase 1 (Arena M8) extends a store that already has the `eval_runs.arq_job_id` socket and dispatches through an MCP harness that **already ships 7 tools** (`build_mcp_server()` is live, not stubbed). Phase 3's reward substrate (all 7 `fieldkit.eval` verifiers) is present. Genuinely absent: the dispatcher glue, the cron layer, hook *expansion* (one `SessionStart` hook today), and the RLVR stack (`fieldkit.reward`/`fieldkit.rl`/`g6_*`). **The Phase-1 / Arena M8 spec is now written** (`_SPECS/spark-arena-v1.md` §12 — the `jobs` table + MCP-harness dispatcher, `eval_rerun` first job type, `/arena/jobs/` surface; spec-only, unbuilt, release gate `fieldkit v0.16.0`). The remaining stubs (`rlvr-loop-v1`, `autonomous-harness-v1`, `second-brain-pipeline-v1`, `cost-plane-v1`) are named here, not written.
